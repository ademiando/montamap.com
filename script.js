...
map = new mapboxgl.Map({ container: 'map',              // pastikan ada <div id="map"></div> di tab Maps style: 'mapbox://styles/mapbox/outdoors-v12', center: [116.4575, -8.4111],   // Koordinat Rinjani zoom: 9 });
...
// 1) MENU TOGGLE if (menuToggle && dropdownMenu) { menuToggle.addEventListener('click', () => dropdownMenu.classList.toggle('menu-visible') );
...
// 2) LOGIN DROPDOWN if (loginButton && loginDropdown) { loginButton.addEventListener('click', () => { loginDropdown.style.display = loginDropdown.style.display === 'block' ? 'none' : 'block'; });
...
// 3) LANGUAGE & CURRENCY PERSISTENCE languageSelect.value = localStorage.getItem('language') || 'en'; currencySelect.value = localStorage.getItem('currency') || 'usd';
...
// 4) THEME TOGGLE function setTheme(mode) { document.documentElement.classList.toggle('dark', mode === 'dark'); localStorage.setItem('theme', mode);
...
// 5) TAB NAVIGATION function openTab(event, tabName) { // Hide all document.querySelectorAll('.tab-content').forEach((c) => { c.style.display = 'none'; }); document.querySelectorAll('.tab').forEach((t) => { t.classList.remove('active'); });
...
// Favorite tab if (tabName === 'Favorite') { renderFavorites(); }

// Maps tab if (tabName === 'Maps') { setTimeout(() => { initMap(); if (map) map.resize(); }, 100); } }

// Bind tabs on load document.addEventListener('DOMContentLoaded', () => { // Apply saved theme setTheme(localStorage.getItem('theme') || 'light');

// Attach click untuk semua .tab document.querySelectorAll('.tab').forEach((btn) => { btn.addEventListener('click', (e) => { const tabName = btn.textContent.trim(); openTab(e, tabName); }); });

// Trigger default visible (tab.active) const defaultTab = document.querySelector('.tab.active'); if (defaultTab) defaultTab.click();
...
async function fetchWeather(lat, lon) { try { const res = await fetch( https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon} + &appid=${apiKey}&units=metric ); const d = await res.json(); if (d.main) { return { temperature: ${Math.round(d.main.temp)}°C, weather: d.weather[0].main, icon: d.weather[0].icon }; } return { temperature: 'N/A', weather: 'N/A', icon: '' }; } catch { return { temperature: 'N/A', weather: 'N/A', icon: '' }; } }
...
function toggleFavorite(id) { let favs = getFavorites(); if (favs.includes(id)) { favs = favs.filter((x) => x !== id); } else { favs.push(id); } saveFavorites(favs);

// Rerender loaded = 0; mountainContainer.innerHTML = ''; renderMountains(); }

async function renderMountains() { const slice = mountainData.slice(loaded, loaded + batch); for (const m of slice) { const w = await fetchWeather(m.lat, m.lon); mountainContainer.appendChild(createMountainCard(m, w)); } loaded += batch;

if (loaded >= mountainData.length) { loadMoreBtn.style.display = 'none'; } }

function createMountainCard(m, w) { const card = document.createElement('div'); card.className = 'mountain-card';

card.addEventListener('click', () => { window.location.href = https://montamap.com/${m.link}; });
...
const favIcon = card.querySelector('.favorite-icon'); favIcon.addEventListener('click', (e) => { e.stopPropagation(); toggleFavorite(m.id); favIcon.textContent = isFavorite(m.id) ? '★' : '☆'; favIcon.title = isFavorite(m.id) ? 'Unfavorite' : 'Favorite'; });

return card; }

async function renderFavorites() { favoriteContainer.innerHTML = ''; const favs = getFavorites(); if (!favs.length) { favoriteContainer.textContent = 'No favorites yet.'; return; }

for (const id of favs) { const m = mountainData.find((x) => x.id === id); if (!m) continue; const w = await fetchWeather(m.lat, m.lon); favoriteContainer.appendChild(createMountainCard(m, w)); } }
